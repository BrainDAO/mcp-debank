...
3. **The filter was dead in the new default surface.** Under v0.2+ the default tools are `execute` and `search_docs` only. The four extra tools (`debank_resolve`, `list_endpoints`, `get_endpoint_schema`, `invoke_endpoint`) are opt-in via `--tools=dynamic`. None of them invoke the v0.1 LLM filter. The filter only fired on `--legacy-tools` paths with `_userQuery` set — a v0.1 affordance for a v0.1-only problem.
